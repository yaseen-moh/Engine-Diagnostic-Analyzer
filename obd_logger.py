...
from config import (
    DATA_DIR, OBD_PORT, OBD_BAUDRATE, OBD_TIMEOUT, SENSOR_RATES
)
...
class OBDDataLogger:
    ...
    def _init_simulation(self) -> Dict:
        ...
        return {
            ...
            "distance_mil": 0,
            "baro": 101.3,
            "trend": "normal",  # normal, degrading, failing
            "trend_timer": 0,
        }
    ...
    def _read_simulated(self, sensor_name: str) -> SensorReading:
        ...
        if s["trend"] == "normal":
            ...
            # Water pump bearing failing: coolant temp slowly rises, voltage drops slightly, RPM jitter
            s["coolant_temp"] += 0.05 + random.gauss(0, 0.1)
            s["voltage"] -= 0.002 + random.gauss(0, 0.02)
            s["rpm"] += random.gauss(0, 25)  # Increased jitter
        ...
        # Map sensor name to simulated value
        value_map = {
            "RPM": s["rpm"],
    ...
    def get_latest_readings(self, n: int = 1) -> Dict[str, List[SensorReading]]:
        ...
        # Simple in-memory cache could be added here
...
def main():
    ...
    parser.add_argument("--vehicle-id", required=True, help="Vehicle identifier (e.g., 'honda-civic-2019')")
    ...
    parser.add_argument("--port", default=OBD_PORT, help="OBD serial port")
